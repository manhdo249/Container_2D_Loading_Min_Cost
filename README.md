...
This is a Project for topic 9 in Optimal Planning course of SoICT - HUST
...
## Problem
`K` trucks `1, 2, ..., K` transport `N` packages `1, 2, ..., N` (2D shape). Truck `k` has the container size of `Wk x Hk`. Package `i` has size `wi x hi`. Packages loaded in a truck can not overlap. `Ck` represents the cost of using truck `k`. Find a solution that loads N packages into K trucks such that **the total cost of trucks used is minimal**.  

*Throughout our Project, some concepts are also being used instead of trucks (bins, cars) and packages (items)*
