...
void Solve_guillotine_partial(vector<Items*>& list_items, vector<Bins*>& list_bins)
{
    for(auto& pack : list_items)
    {
        // Bin First Fit: choose bin that first fit
        for(auto& b : list_bins)
            if(insert_item_guillotine(*b, *pack)) break;
    }
}

/*---------------- LOCAL SEARCH ------------------*/

// Add declarations for Local search and Simulated annealing
pair<Bins*, Bins*> calculate_top_empty_space(const vector<Bins>& bins); // Computes the total unused area across all bins.
vector<Bins> local_search(vector<Bins> current_bins,int max_iter);


int calculate_score(vector<Bins*> list_bins){
    int smallest_empty_space = INT_MAX;
    for (const auto & b : list_bins){
        if (b->free_area < smallest_empty_space) smallest_empty_space = b->free_area;
    }
...
vector<Bins*> calculate_top_and_random_bins(vector<Bins>& bins, int K) {
    ...
    if (top1_bin != nullptr) {
        ...
        while (selected_bins.size() < K + 1 && !other_bins.empty()) {
            int index = std::rand() % other_bins.size();
...
vector<Bins> local_search(vector<Bins> current_bins,int max_iter) {
    vector<Bins> best_bins = current_bins;

    while(max_iter--) {

        // Select the top 2 bins
        // pair<Bins*, Bins*> top_bins = calculate_top_empty_space(best_bins);
        // vector<Bins*> list_bins = sort_bins(top_bins);
...
// vector<Bins> simulated_annealing(const vector<Bins>& initial_bins, int max_iterations, double initial_temperature, double cooling_rate) {
//     vector<Bins> current_bins = initial_bins;
//     vector<Bins> best_bins = current_bins;
//     double current_score = calculate_empty_space(current_bins);
...
//         // Generate a neighbor solution by swapping two random items
//         vector<Bins> new_bins = current_bins;

//         // Select two random bins
//         if(new_bins.empty()) break;
//         uniform_int_distribution<> bin_dis(0, new_bins.size() - 1);
//         int bin_idx = bin_dis(gen);
//         if(new_bins[bin_idx].list_of_items.size() < 2) continue;

//         // Select two random items within the bin
//         uniform_int_distribution<> item_dis(0, new_bins[bin_idx].list_of_items.size() - 1);
//         int item1_idx = item_dis(gen);
//         int item2_idx = item_dis(gen);
//         while(item2_idx == item1_idx) {
//             item2_idx = item_dis(gen);
//         }

//         // Swap the two items
//         swap(new_bins[bin_idx].list_of_items[item1_idx], new_bins[bin_idx].list_of_items[item2_idx]);

//         // Calculate the new score
//         double new_score = calculate_empty_space(new_bins);
...
//             }
//         } else {
//             double acceptance_prob = exp((current_score - new_score) / temperature);
...
//         if(temperature < 1e-8) break; // Avoid temperature becoming too low
//     }
