...
bool compare_ranking_rec_BSS(const std::pair<T1, T2>& a, const std::pair<T1, T2>& b) {
    if (a.first == b.first) {
...
// Calculate score * cost for the item when insert to a free_rec in best fit stratergy
pair <double, double> score_rec_best_fit(Free_Rectangles rec, Items pack, bool rotated, int cost, bool empty)
{
    ...
    if(rotated)
    {
        score.first     = (min(rec.width - pack.height, rec.height - pack.width) + cost) * cost; // avoid multiply by zero
        score.second    = (max(rec.width - pack.height, rec.height - pack.width) + cost) * cost;
    }
    // Not rotate case
    else
    {
        score.first     = (min(rec.width - pack.width, rec.height - pack.height) + cost) * cost;
        score.second    = (max(rec.width - pack.width, rec.height - pack.height) + cost) * cost;
    }
    if (!empty){  // if not a new car, then process like normal
        score.first /= cost;
        score.first -= cost;
        score.second /= cost;
        score.second -= cost;
...
void Solve_maxrec_best_fit(){
    for(int i = 1; i <= N_items; ++i)
    {
        // Bin Best Fit
        int best_bin_id = 0;
        pair <double, double> best_score = {99999, 99999};
        for(int j = 1; j <= N_bins; ++j){
            pair <pair <Free_Rectangles, unsigned int>, pair <bool, bool>> best_ranking_return;
            best_ranking_return = best_ranking(bin[j], item[i]);

            if(!best_ranking_return.second.second) continue;

            Free_Rectangles best_rec    = best_ranking_return.first.first;
            int rotated                 = best_ranking_return.second.first;
            if (compare_ranking_rec_BSS(score_rec_best_fit(best_rec, item[i], rotated, bin[j].cost, bin[j].list_of_items.empty()), best_score)){
                best_bin_id = j;
                best_score = score_rec_best_fit(best_rec, item[i], rotated, bin[j].cost, bin[j].list_of_items.empty());
            }
        }
        insert_item_maxrec(bin[best_bin_id], item[i]);
    }
}

bool Solve_maxrec_best_fit_partial(vector<Items*>& list_items, vector<Bins*>& list_bins){
    for(auto& pack : list_items)
    {
        // Bin Best Fit
        bool found_place = false;
        Bins* best_bin_ptr;
        pair <double, double> best_score = {999999, 999999};
        for(auto& b : list_bins){
            pair <pair <Free_Rectangles, unsigned int>, pair <bool, bool>> best_ranking_return;
            best_ranking_return = best_ranking(*b, *pack);

            if(!best_ranking_return.second.second) continue;
            found_place = true;
            Free_Rectangles best_rec    = best_ranking_return.first.first;
            int rotated                 = best_ranking_return.second.first;
            if (compare_ranking_rec_BSS(score_rec_best_fit(best_rec, *pack, rotated, b->cost, b->list_of_items.empty()), best_score)){
                best_bin_ptr = b;
                best_score = score_rec_best_fit(best_rec, *pack, rotated, b->cost, b->list_of_items.empty());
            }
        }
        if (!found_place) return false;
        insert_item_maxrec(*best_bin_ptr, *pack);
    }
    return true;
}
...
void Solve_guillotine_best_fit(){
    for(int i = 1; i <= N_items; ++i)
    {
        // Bin Best Fit
        int best_bin_id = 0;
        ...
        insert_item_guillotine(bin[best_bin_id], item[i]);
    }
}

bool Solve_guillotine_best_fit_partial(vector<Items*>& list_items, vector<Bins*>& list_bins){
    for(auto& pack : list_items)
    {
        // Bin Best Fit
        bool found_place = false;
        ...
        if (!found_place) return false;
        insert_item_guillotine(*best_bin_ptr, *pack);
    }
    return true;
}
...
vector<Bins*> calculate_top_and_random_bins(vector<Bins>& bins, int K) {
    ...
    if (top1_bin != nullptr) {
        ...
        while ((int)selected_bins.size() < K && !other_bins.empty()) {
...
void local_search(vector<Bins>& current_bins,int max_iter) {
    while(max_iter--) {
        ...
        bool found_sol = Solve_maxrec_best_fit_partial(list_items, list_bins);
